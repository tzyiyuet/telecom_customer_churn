...
telecommunication = telecommunication.apply(pd.to_numeric)
...
tele_dummy = tele_yn[column_name[1:17]]
tele_dummy['Churn'] = tele_yn['Churn']
...
# logistic model
logistic_x_name = tele_new.drop(['customerID','Churn','tenure','OnlineSecurity_No internet service', 'OnlineBackup_No internet service','DeviceProtection_No internet service','TechSupport_No internet service','StreamingTV_No internet service','StreamingMovies_No internet service'], axis = 1)
...
print(model1.score(logistic_x,logistic_y))

model_col_name = list(logistic_x_name)
coef1 = coef.tolist()
coef_value = list(itertools.chain.from_iterable(coef1))

model_data_list = [coef_value]
model_table = pd.DataFrame(model_data_list, columns = model_col_name)
model_table = model_table.transpose()
model_table.columns = ['Values']

#model_table.to_csv('C:/Users/PC/Downloads/model.csv')
